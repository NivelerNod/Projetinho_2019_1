...
Onde a função ```faz_multinomial(self, pvalores, pcv)``` realiza a aleatoriedade da distribuição, seguindo cada ```dado``` e cada ```cv_dado``` que representa cada atributo da pessoa.  
  
  Obs.: os arquivos "pessoa.py" e "pessoa_nutella.py" são exemplos de como **NÃO SE DEVE FAZER** o que foi proposto pelo projeto, pois seus códigos foram feitos sem a devida consideração do coeficientes de variação.  
Procurei deixá-los para quem quizer saber como foi a ideia inicial do projeto.